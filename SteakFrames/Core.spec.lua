...
if class == "WARLOCK" then
	CanDispel = { Magic = true, Curse = true }
elseif class == "PRIEST" then
	CanDispel = { Magic = true, Disease = true }
elseif class == "PALADIN" then
	CanDispel = { Magic = true, Poison = true, Disease = true }
...
local SteakInspectFrame = CreateFrame("Frame", nil, UIParent)
]]
--[[
NotifyInspect = function(unit)
	if UnitIsUnit("player", unit) then return end

	if not SteakInspectUnitGUID and unit and UnitIsPlayer(unit) and CanInspect(unit) and CheckInteractDistance(unit, 1) then
		SteakInspectUnitGUID = UnitGUID(unit)

		SteakInspectFrame:SetScript("OnUpdate", function(self, elapsed)
	...
	--else
		--print("Already inspecting a unit.")
	end
...
local function Steak_GetSpec(self)
	...
	if guid ~= SteakInspectUnitGUID and not UnitIsUnit(self.unit, "player") then return nil end
	...
	for tab=1,GetNumTalentTabs() do
		...
		if UnitIsUnit(self.unit, "player") then
			...
			name, _, points = GetTalentTabInfo(tab, true, nil, self.unit)
		end
	...
	SteakTalents[guid] = table.concat(talents, "/")
	--print(UnitName(self.unit), self.unit, specName, table.concat(talents, "/"))
	--self.spec = specName
	--self.talents = table.concat(talents, "/")
	return specName, table.concat(talents, "/")
end
...
local function Steak_UnitAura(self)
	if UnitIsFriend("player", self.unit) then
		...
		for i=1,40 do
			local name, _, _, _, debuffType = UnitDebuff(self.unit, i)

			if debuffType and DebuffColors[debuffType] and CanDispel[debuffType] then
...
local function Steak_OnEvent(self, event, ...)
	...
	elseif event == "INSPECT_TALENT_READY" then
		if SteakInspectUnitGUID and UnitGUID(self.unit) == SteakInspectUnitGUID then
			local spec, talents = Steak_GetSpec(self)
			Steak_UpdateRole(self)
			
			if spec then
				self.specText:SetText(("%s\n%s"):format(spec:sub(1, 5), talents))
				SteakInspectUnitGUID = nil
			else
				self.specText:SetText("")
			end
	...
	--else
		...
		if UnitExists(self.unit) then
			...
			--if self.spec then
				self.specText:SetText(("%s\n%s"):format(SteakSpecs[UnitGUID(self.unit)]:sub(1, 5) or "???", SteakTalents[UnitGUID(self.unit)] or "?/?/?"))
				--self.specText:SetText(("%s\n%s"):format(self.spec:sub(1,5), self.talents))
			end
...
local function CreateSteakUnitFrame(name, unit, width, height, parent)
	...
	local debuff = frame:CreateTexture(nil, "ARTWORK")
	debuff:SetAllPoints(frame)
	debuff:SetTexture(0, 0, 0, 0)
	frame.debuff = debuff
