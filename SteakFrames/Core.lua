...
local function ReanchorPlayerFrame()
    ...
    PlayerFrame:SetPoint("BOTTOM", UIParent, "BOTTOM", -150, 220)
end
...
local function MoveUnitFrames()
	...
	ApplyClassColor(PlayerFrame, "player")

    -- Target Frame
    TargetFrame:SetMovable(true)
    TargetFrame:ClearAllPoints()
    TargetFrame:SetPoint("BOTTOM", UIParent, "BOTTOM", 150, 220)
    TargetFrame:SetUserPlaced(true)
	ApplyClassColor(TargetFrame, "target")

    -- Focus Frame
    FocusFrame:SetMovable(true)
    FocusFrame:ClearAllPoints()
    FocusFrame:SetPoint("BOTTOM", UIParent, "BOTTOM", -250, 340)
    FocusFrame:SetUserPlaced(true)
	ApplyClassColor(FocusFrame, "focus")
	...
	--ApplyClassColor(TagetFrameToT, "targettarget")
	...
	for i=1,4,1 do
		...
		if frame then
			...
			if i == 1 then
				frame:SetPoint("BOTTOM", UIParent, "BOTTOM", -275, 320)
			else
				frame:SetPoint("LEFT", _G["PartyMemberFrame"..(i-1)], "RIGHT", 40, 0)
			end
			frame:SetUserPlaced(true)
			ApplyClassColor(frame, "party"..i)
		end
	end
end
...
local function OnUpdate(self, elapsed)
    ...
    if self.timer >= 0.5 then
        if GetNumRaidMembers() > 0 then
            for i=1,25 do
                local unit = "raid"..i
                local unitFrame = _G["SteakRaid"..i]
...
hooksecurefunc("CastingBarFrame_OnShow", function(self)
    ...
    if GetNumPartyMembers() > 0 then
        self:SetPoint("BOTTOM", UIParent, "BOTTOM", 0, 390)
    else
...
hooksecurefunc("HealthBar_OnValueChanged", function(self, value, smooth)
    if self.lockColor then return end

    local parent = self:GetParent()
    local unit = parent:GetUnit()
...
hooksecurefunc("PlayerFrame_UpdateLayout", function(self)
    ReanchorPlayerFrame()
end)